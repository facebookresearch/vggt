...
def build_argparser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--verbose", action="store_true", help="Print diagnostics and sample matches")
    return p
...
def main(argv=None) -> int:
    ...
    if not cams_dir.exists():
        raise SystemExit(f"Cameras directory not found: {cams_dir}")

    files = sorted(cams_dir.glob(args.glob))
    if not files:
        # Try a few fallback patterns commonly used by PKU dumps
        fallback_patterns = ["*_cam.txt", "*.txt", "*_cam*.txt"]
        tried = [args.glob]
        for pat in fallback_patterns:
            if pat in tried:
                continue
            cand = sorted(cams_dir.glob(pat))
            if cand:
                files = cand
                if args.verbose:
                    print(f"No matches for '{args.glob}'. Falling back to '{pat}' with {len(files)} files.")
                break
            tried.append(pat)
        if not files:
            sample_any = sorted(cams_dir.glob("*"))[:5]
            hint = "(quote the pattern to avoid shell expansion, e.g., --glob \"*_cam.txt\")"
            raise SystemExit(
                "No camera files matched. Tried patterns: "
                + ", ".join(tried)
                + f" under {cams_dir}. {hint}. Sample entries: "
                + ", ".join(p.name for p in sample_any)
            )

    if args.verbose:
        print(f"Cameras dir: {cams_dir.resolve()}")
        print(f"Using pattern: {args.glob} -> {len(files)} files")
        for name in [f.name for f in files[:3]]:
            print(f" - {name}")
