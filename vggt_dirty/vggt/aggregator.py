...
from off3d.models.vggt.rope import RoPE2D, PositionGetter
...
from off3d.models.dino_layers.vision_transformer import vit_small, vit_base, vit_large, vit_giant2
...
class Aggregator(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        # we need to hard code the torchhub dir
        if os.path.exists("/fsx-repligen/shared/datasets/co3d/"):
            torch.hub.set_dir('~/.cache/torch/hub/')
        else:
            torch.hub.set_dir('/checkpoint/repligen/jianyuan/torchhub/hub')
        ...
        # if "postnorm" in embed_type:
            # self.embed_norm = nn.LayerNorm(normalized_shape=aa_block_kwargs['dim'], eps=1e-6, elementwise_affine=False)
