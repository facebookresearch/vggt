...
from .blocks import EfficientUpdateFormer, CorrBlock, NewCorrBlock
from .utils import sample_features4d, get_2d_embedding, get_2d_sincos_pos_embed
from .modules import Mlp


class BaseTrackerPredictorV2(nn.Module):
    def __init__(
        self,
        stride=1,
        corr_levels=5,
        ...
        super(BaseTrackerPredictorV2, self).__init__()
        """
        ...
        self.corr_mlp = Mlp(in_features=self.corr_levels * (self.corr_radius * 2 + 1) ** 2, 
                                    hidden_features=self.hidden_size, out_features=self.latent_dim)
    ...
    def forward(
        self, query_points, fmaps=None, iters=6, return_feat=False, down_ratio=1
    ):
        """
        ...
        for _ in range(iters):
            ...
            query_ref_token = torch.cat([self.query_ref_token[:,0:1], self.query_ref_token[:,1:2].expand(-1, S-1, -1)], dim=1)
            ...
            # if self.use_checkpoint:
                # delta = checkpoint(self.updateformer, x, use_reentrant=False)
            # else:
